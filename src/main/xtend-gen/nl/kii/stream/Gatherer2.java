...
public class Gatherer2<T extends Object> implements Streamable<Pair<String, T>> {
  ...
  public Gatherer2() {
    final Procedure1<Pair<String, T>> _function = new Procedure1<Pair<String, T>>() {
      public void apply(final Pair<String, T> it) {
        ...
        Gatherer2.this.data.put(_key, _value);
      }
    };
  ...
  public Procedure1<? super T> await(final String name) {
    ...
    final Procedure1<T> _function = new Procedure1<T>() {
      public void apply(final T it) {
        ...
        Gatherer2.this.stream.push(_mappedTo);
        int _incrementAndGet = Gatherer2.this.count.incrementAndGet();
        int _get = Gatherer2.this.total.get();
        boolean _equals = (_incrementAndGet == _get);
        if (_equals) {
          Gatherer2.this.stream.finish();
        }
      }
    };
    return _function;
  ...
  public Stream<Pair<String, T>> onFinish(final Procedure1<? super Map<String, T>> closure) {
    final Procedure1<Void> _function = new Procedure1<Void>() {
      public void apply(final Void it) {
        ConcurrentHashMap<String, T> _result = Gatherer2.this.result();
