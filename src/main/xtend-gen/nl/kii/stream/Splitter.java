...
public abstract class Splitter<T extends Object> implements Source<T> {
  /**
  ...
  public Splitter(final Stream<T> source) {
    this.source = source;
  ...
  public Stream<T> stream() {
    ...
    final Procedure1<Stream<T>> _function = new Procedure1<Stream<T>>() {
      public void apply(final Stream<T> it) {
        Splitter.this.pipe(it);
      }
    };
    return ObjectExtensions.<Stream<T>>operator_doubleArrow(_stream, _function);
  }
  
  public Source<T> pipe(final Stream<T> stream) {
    Splitter<T> _xblockexpression = null;
    {
