 ...
 */
class StreamOLDEST<T> implements Procedure1<Entry<T>> {
	...
	new(StreamOLDEST<?> parentStream, =>Queue<Entry<T>> queueFn) { 
	 ...
	 * in a stream chain.
	 */
	def synchronized error(Throwable t) {
		apply(new Error(t))
		publish
		null
	}
	...
	override synchronized apply(Entry<T> entry) {
	 ...
	 */
	package def synchronized void skip() {
	...
	package def synchronized void next() {
		readyForNext = true
		publish
	}
	...
	package def synchronized void publish() {
	 ...
	 */
	protected def synchronized boolean publish(Entry<T> entry) {
		switch it : entry {
			Value<T>: {
